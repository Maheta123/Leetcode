...
int set[N][2],ori[N][2],temp[N];//segtree[(1<<19)];
int max(int a,int b){
    return ((a>=b)?(a):(b));
}
int min(int a,int b){
...
int maxFrequency(int* nums, int numsSize, int k, int numOperations) {
    ...
    // prefix Sum InclStart-ExclEndPlus Technique to simulate line 
    for(i=0;i<sz;i++){
        temp[z]=ori[i][0]-k;
        temp[++z]=ori[i][0]+k+1;
        z++;
        ans = max(ans,ori[i][1]);
        update(i,ori[i][1],&stsz); // update happens :- T.C:- log(n)
    }
    ...
    // binarySeaarch simulating set insertion operation 
    for(i=0;i<n;i++){
    ...
    // same prefixSum approach as previous problem 
    for(int i=0;i<ssz;i++){
        if(i==0) {
           continue;// goto label;
        }
